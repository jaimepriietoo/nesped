 ...
 *  - Inactividad: una sesión de owner o admin que lleve más de quince
 *    minutos sin usarse se cierra sola. Las demás caducan a los siete días.
 */

const INACTIVIDAD_ADMIN_MS = 15 * 60 * 1000;
const RETOQUE_MINIMO_MS = 60 * 1000;
const ROLES_ESTRICTOS = new Set(["owner", "admin"]);
 ...
 * motivo si no. Nunca lanza: un fallo de la base no debe dejar a nadie fuera
 * por sí solo... salvo que la fila no exista, que es exactamente lo que se
 * quiere comprobar.
 */
export async function comprobarSesion({ sid, email, clientId, role, ahora = Date.now() }) {

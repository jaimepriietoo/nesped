...
export const POR_DEFECTO = Object.freeze({
  ...
  mensaje_bienvenida: "",
  mensaje_fuera_horario: "Ahora mismo estamos fuera de horario. Déjanos tu nombre y qué necesitas y te llamamos en cuanto abramos.",
  reglas_derivacion: [],
  instrucciones: "",
});
...
export function promptDeEmpresa(config, { empresa = "", sector = "", departamentos = [] } = {}) {
  ...
  const lineas = [
    ...
    departamentos.length ? `DEPARTAMENTOS: ${departamentos.map((d) => `${d.nombre}${d.descripcion ? ` (${d.descripcion})` : ""}`).join("; ")}.` : "",

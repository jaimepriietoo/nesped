 ...
 *
 * Nesped depende de cuatro empresas que se caen: Resend para el correo, Twilio
 * para el teléfono, ElevenLabs para la conversación y Stripe para el cobro. Cuando
...
async function estadoDe(proveedor) {
  ...
  const { data } = await getSupabaseAdministrativo()
    .from("cortacircuitos")
 ...
 *
 * @param proveedor    Nombre corto: "resend", "twilio", "elevenlabs", "stripe".
...
export async function conCortacircuitos({ proveedor, quienEspera = "nadie", umbral, espera }, fn) {
  ...
  try {
    ...
    if (estado && (estado.fallos_seguidos > 0 || estado.abierto_hasta)) {
      ...
      await getSupabaseAdministrativo().rpc("anotar_acierto_proveedor", { p_proveedor: proveedor });
    }
    ...
    try {
      await getSupabaseAdministrativo().rpc("anotar_fallo_proveedor", {

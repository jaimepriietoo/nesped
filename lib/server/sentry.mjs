...
const sanitizeHeaders = redactData;
const sanitizeObject = redactData;

function beforeSend(event) {
  const nextEvent = redactData(event);

  if (nextEvent.request) {
    nextEvent.request = { ...nextEvent.request };

    if (nextEvent.request.headers) {
      nextEvent.request.headers = sanitizeHeaders(nextEvent.request.headers);
    }
    ...
    if (nextEvent.request.data) {
      nextEvent.request.data = "[redacted]";
    }
...
export function isServerSentryEnabled() {
  /* Con DSN y en producción, o en un entorno de Vercel/Railway que no sea
     local. En un `next dev` en el portátil no se manda nada: los errores de
     desarrollo se ven en la terminal y sólo ensucian el panel. */
  if (!getServerSentryDsn()) return false;
  if (process.env.NODE_ENV === "production") return true;
  return Boolean(process.env.VERCEL_ENV || process.env.RAILWAY_ENVIRONMENT);
}
...
export async function captureServerException(error, context = {}) {
  ...
  return Sentry.withScope((scope) => {
    /* Lo que haya en el contexto de la operación —petición, trabajo,
       empresa— va como etiquetas: es lo que permite buscar en Sentry por
       request_id o por empresa. */
    try {
      const ctx = contextoActual();
      for (const k of ["request_id", "job_id", "job_tipo", "client_id", "route", "call_id"]) {
        if (ctx[k] !== undefined && ctx[k] !== null) scope.setTag(k, String(ctx[k]).slice(0, 200));
      }
    } catch {}

 ...
 * la conversación (evento datos_de_llamada); `lead` sólo aporta el
 * departamento al que se ha derivado.
 */
export function correoDeLlamada({ empresa, llamada, lead, dicho = {}, destinatario, urlPortal }) {
  const cuando = llamada.created_at ? new Date(llamada.created_at).toLocaleString("es-ES", { timeZone: "Europe/Madrid" }) : "";
  const asunto = `Llamada en ${empresa}: ${dicho.nombre || llamada.from_number || "número oculto"} · ${Math.round(Number(llamada.duration_seconds || 0))} s`;
  const filas = [
    ...
    ["Ha dicho llamarse", dicho.nombre], ["Localidad que ha dado", dicho.ciudad], ["Lo que necesita", dicho.necesidad],
    ["Preferencia", dicho.preferencia], ["Notas de la llamada", dicho.notas],
    ["Derivado a", lead?.departamento], ["Por qué", lead?.departamento_motivo],
  ].filter(([, v]) => v);
...
export async function notificarLlamada({ clientId, callSid, supabase = getSupabase() }) {
  ...
  try {
    ...
    /* Lo que la IA recogió en ESTA conversación, no la ficha. */
    const { data: eventos } = await supabase.from("lead_events").select("meta")
      .eq("client_id", clientId).eq("type", "datos_de_llamada")
      .filter("meta->>conversation_id", "eq", callSid)
      .order("created_at", { ascending: false }).limit(1);
    const dicho = eventos?.[0]?.meta || {};
    ...
    for (const destinatario of destinatarios) {
      ...
      try {
        const { asunto, html } = correoDeLlamada({ empresa: nombreEmpresa, llamada, lead, dicho, destinatario, urlPortal });
        const id = await enviarCorreo({ to: [destinatario.email], subject: asunto, html });
        resumen.enviados += 1;
        await supabase.from("notificaciones_lead").insert({ ...fila, estado: "enviado", proveedor_id: id || null });
        resumen.destinatarios.push({ email: destinatario.email, estado: "enviado" });

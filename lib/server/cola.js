 ...
 */
export async function encolar({ tipo, clientId = null, datos = {}, clave = null, unaSolaVez = false, noAntesDe = null }) {
  ...
  const { data, error } = await getSupabase()
    .from("trabajos")
    .insert({ tipo, client_id: clientId, datos, clave_unica: clave, ...(noAntesDe ? { no_antes_de: noAntesDe } : {}) })
    .select("id")
    .single();
 ...
 * Lo da por hecho.
 *
 * @param aviso  Opcional: algo que salió regular sin llegar a tumbar el
 *   trabajo. Se guarda en la misma columna que los errores para que quien mire
 *   la cola lo vea sin tener que ir a buscarlo a otro sitio. Un trabajo que
 *   sale en verde escondiendo que la mitad no se hizo es peor que uno rojo.
 */
export async function terminar(id, aviso = null) {
  await getSupabase()
    .from("trabajos")
    .update({
      estado: "hecho",
      terminado_en: new Date().toISOString(),
      error: aviso ? String(aviso).slice(0, 500) : null,
    })
    .eq("id", id);
}
...
/**
 * Cuántos trabajos llevan pendientes más de lo razonable.
 *
 ...
 * purgas simplemente no salen. Un trabajo pendiente y ya vencido desde hace
 * más de quince minutos es la señal de que nadie está empujando.
 */
export async function latidoDeLaCola(minutos = 15, supabase = getSupabase()) {
  const limite = new Date(Date.now() - minutos * 60_000).toISOString();
  const { data, error } = await supabase
    .from("trabajos")
    .select("creado_en")
    .eq("estado", "pendiente")
    .lt("no_antes_de", limite)
    .order("creado_en", { ascending: true })
    .limit(50);
  if (error) return { comprobado: false, motivo: error.message };
  const masAntiguo = data?.[0]?.creado_en
    ? Math.round((Date.now() - new Date(data[0].creado_en)) / 60_000)
    : 0;
  return { comprobado: true, minutos, pendientesViejos: (data || []).length, masAntiguoMin: masAntiguo };
}

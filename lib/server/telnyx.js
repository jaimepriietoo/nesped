...
/**
 * Toda llamada a Telnyx pasa por aquí, y por eso el cortacircuitos va aquí.
 *
 * Telnyx es lo que hace que suene un teléfono. Cuando falla, los automatismos
 * y la cola reintentan, y cada reintento es una petición más contra un
 * servicio caído, facturada igual que si hubiera funcionado.
 *
 * @param quienEspera "persona" cuando alguien está mirando la pantalla ahora
 *   —el SMS del segundo factor—; "nadie" para todo lo demás.
 */
async function sendTelnyxRequest(path, payload, { quienEspera = "nadie" } = {}) {
  const apiKey = getTelnyxApiKey();
  if (!apiKey) {
    throw new Error("Falta TELNYX_API_KEY");
  }

  return conCortacircuitos({ proveedor: "telnyx", quienEspera }, async () => {
    ...
    /* Un 4xx que no sea 429 lo hemos provocado nosotros: un número mal
       escrito, un campo que falta, una cuenta sin saldo. Telnyx está
       perfectamente. Contarlo como caída abriría el circuito por un solo
       contacto con el teléfono mal metido y dejaría a toda la plataforma sin
       mandar mensajes. */
...
export async function sendTelnyxSms({
  ...
  return sendTelnyxRequest("/v2/messages", body, { quienEspera });
}

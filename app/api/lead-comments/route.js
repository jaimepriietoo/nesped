...
async function manejarGET(req) {
  try {
    ...
    logErrorSeguro("lead_comments.read_failed", err);
...
async function manejarPOST(req) {
  try {
    ...
    const limite = await requireRateLimitAsync(req, {
      namespace: "lead-comments", limit: 100, keyParts: [ctx.clientId, ctx.userEmail],
    });
    ...
    const { lead_id, body: commentBody } = entrada.datos;
    ...
    if (ajeno) return ajeno;
    const { data, error } = await ctx.supabase.from("lead_comments").insert({
      lead_id,
      body: commentBody,
    ...
    logErrorSeguro("lead_comments.write_failed", err);
    return Response.json({ success: false, message: "No se pudo completar la operación" }, { status: 500 });
  }
}

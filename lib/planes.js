 ...
 *
 *   Crecimiento   Nesped ordena y automatiza tu negocio.
 *   Inteligencia  Nesped entiende tu negocio y te dice qué hacer.
 *   Empresa       Nesped trabaja por tu negocio.
 *
 ...
 * repetir listas: cuando se añade una función a Crecimiento aparece sola en los
...
export const PLANES = {
  ...
  intelligence: {
    ...
    descripcion:
      "Todo lo de Crecimiento, y encima una capa que mira tus datos y señala dónde está el dinero.",
    precio: 999,
...
/* ── Límites de consumo ────────────────────────────────────────────────
   Cada llamada cuesta dinero real en minutos de teléfono y en voz sintética.
   Sin un tope, un solo cliente puede consumir el presupuesto de todos: una
   centralita mal desviada, un bucle de rellamada o sencillamente un negocio
   que crece más de lo previsto.

   No son límites de castigo. Están holgados a propósito —un negocio normal no
   los roza— y su función es que un caso anómalo se note antes de aparecer en
   la factura, no cortarle el servicio a nadie por usar lo que paga.

   Cuando se pasan, la llamada SE ATIENDE igual y se avisa. Cortar el teléfono
   de un cliente por superar una cuota interna es el peor momento posible para
   descubrir que la cuota estaba mal calculada. */
export const LIMITES = {
  growth: { llamadasMes: 1500, minutosMes: 3000 },
  intelligence: { llamadasMes: 5000, minutosMes: 10000 },
  /* Enterprise se negocia por contrato: el tope se pone por acuerdo, no por
     una tabla. Se deja alto para que el aviso siga existiendo. */
  enterprise: { llamadasMes: 50000, minutosMes: 100000 },
};

/** Los límites del plan de una empresa, con el del plan de entrada si no cuadra. */
export function limitesDe(planId) {
  /* Los topes de consumo no son una función de pago: frenan un bucle de
     llamadas antes de que llegue a la factura. Siguen al plan guardado. */
  return LIMITES[planGuardado({ plan: planId })] || LIMITES.growth;
}
...
/* Desde el 24-09-2026 no hay planes a la venta: cada cliente tiene todas
   las funciones, sin pagar por subir de nivel. Se conserva la definición de
   los planes (y lo que haya guardado en la base) por si se vuelven a usar,
   pero a efectos de acceso todo el mundo cuenta como el plan completo. */
export const ACCESO_TOTAL = "enterprise";
export const PLAN_POR_DEFECTO = ACCESO_TOTAL;

/** El plan con el que se decide el acceso: siempre el completo. */
export function planDe(cliente) {
  return ACCESO_TOTAL;
}

/** Lo que dice la base de datos, normalizado. Sólo informativo. */
export function planGuardado(cliente) {
...
export function planSiguiente(planId) {
  const i = ORDEN_PLANES.indexOf(planGuardado({ plan: planId }));

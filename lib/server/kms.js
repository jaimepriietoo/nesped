 ...
 * Vercel se lleva sobres, y el pase para abrirlos (las credenciales IAM) se
 * revoca en un click y deja rastro en CloudTrail.
 *
...
export function clienteKms(env = process.env) {
  ...
  if (!env.AWS_REGION || !env.AWS_ACCESS_KEY_ID || !env.AWS_SECRET_ACCESS_KEY) {
    throw new Error("Faltan AWS_REGION, AWS_ACCESS_KEY_ID o AWS_SECRET_ACCESS_KEY para abrir sobres KMS");
  }
  cliente = new KMSClient({ region: env.AWS_REGION });
  return cliente;
}
...
export const PARA_PRUEBAS = { PREFIJO, abiertos };

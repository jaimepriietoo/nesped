...
export async function medirFuentes({ supabase, clientId }) {
  const [cliente, llamadas, leads, eventos, equipo, ejecuciones] = await Promise.all([
    supabase
      .from("clients")
    ...
    supabase
      ...
      .eq("client_id", clientId),
    supabase
      .from("automatismos_ejecuciones")
      .select("tipo,resultado,created_at")
      .eq("client_id", clientId)
...
function sinSeguimiento({ leads }) {
  ...
  return {
    ...
    /* Los campos van con nombre completo y unidad porque el copiloto los lee
       tal cual y los cita: con `esperando: 120` acabó escribiéndole a alguien
       "aparece con esperando: 120" en mitad de una frase. */
    items: pendientes.slice(0, 25).map((l) => {
      const h = Math.floor(horas(desde(l.created_at)));
      return {
        id: l.id,
        nombre: l.nombre || "Sin nombre",
        telefono: l.telefono || "",
        necesidad: l.necesidad || "",
        tiempoEsperando: h >= 48 ? `${Math.floor(h / 24)} días` : `${h} horas`,
      };
    }),
  };
}
...
/**
 * Lo que Nesped ha hecho por sí solo esta semana.
 *
 * Sustituye a la cifra de "dinero cerrado este mes": esa dependía de que
 * alguien pusiera importes a mano y con dos operaciones bailaba miles de
 * euros. Esto se cuenta solo: cuántas veces actuó un automatismo, cuántas
 * quedaron esperando a una persona, y cuántos contactos se derivaron.
 */
function loQueHizoNesped({ ejecuciones = [] }) {
  const utiles = ejecuciones.filter((e) => e.resultado !== "omitido");
  if (!utiles.length) {
    return {
      disponible: false,
      titulo: "Lo que hizo Nesped",
      falta: "Todavía no ha actuado ningún automatismo esta semana.",
      porQue: "En cuanto entren contactos y haya automatismos activos, aquí se ve cuánto trabajo se ahorró el equipo.",
    };
  }
  const hechos = utiles.filter((e) => e.resultado === "hecho").length;
  const esperando = utiles.filter((e) => e.resultado === "preparado" || e.resultado === "avisado").length;
  const fallidos = utiles.filter((e) => e.resultado === "fallido").length;
  const derivaciones = utiles.filter((e) => ["derivar_persona", "escalar_direccion", "motivo_derivacion"].includes(e.tipo) && e.resultado === "hecho").length;
  return {
    disponible: true,
    valor: hechos,
    unidad: "acciones",
    titulo: "Lo que hizo Nesped",
    resumen: `${hechos} acciones hechas solas esta semana, ${esperando} esperando a una persona${derivaciones ? `, ${derivaciones} derivaciones` : ""}${fallidos ? ` y ${fallidos} que fallaron` : ""}.`,
    severidad: fallidos > hechos / 4 ? "media" : "bien",
    metodo: "Recuento de las ejecuciones de automatismos de los últimos siete días, por resultado. Cada una está en la pantalla de Automatismos con su detalle.",
  };
}

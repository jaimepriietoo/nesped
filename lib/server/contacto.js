...
function montarRecorrido({ lead, llamadas, eventos, auditoria }) {
  ...
  for (const c of llamadas) {
    ...
    hitos.push({
      ...
      llamadaId: c.id,
      tiene_grabacion: Boolean(c.grabacion_propia),
...
export async function fichaDeContacto({ supabase, clientId, leadId }) {
  ...
  const [llamadas, eventos, auditoria, hermanos] = await Promise.all([
    telefono
      ? supabase
          .from("calls")
          .select("id,created_at,duration_seconds,summary,sentiment,detected_intent,call_outcome,lead_captured,grabacion_propia,transcript")
          .eq("client_id", clientId)
          .or(condicionOr(
            "calls", ["from_number", "phone"], telefono, modoCifrado(), process.env, clientId,
          ))

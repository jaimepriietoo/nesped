import { eventosPorTipo } from "@/lib/server/datos";
...
export async function getPaidLeadRows(limit = 500) {
  const paymentEvents = await eventosPorTipo({ type: "payment_completed", cuantos: limit });
  ...
  return paymentEvents.map((event) => {
    ...
    return {
      ...
      client_id: event.client_id || parsed.client_id || "",

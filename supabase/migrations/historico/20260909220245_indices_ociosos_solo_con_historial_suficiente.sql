...
--
-- Puede ser que nadie lo haya necesitado todavía. idx_calls_call_sid lo usa el
-- servidor de voz para encontrar una llamada por su identificador, y sale con
-- cero usos por el motivo más simple: no hay número de teléfono contratado y
-- no ha entrado ninguna llamada. Borrarlo por esa lectura sería quitar
-- justamente el índice que hará falta el día uno.
--
-- Las estadísticas también se reinician al reiniciar la base de datos, y
-- entonces todo sale a cero durante un rato.
--
-- Así que se mira desde cuándo se está contando. Con menos de una semana de
-- historial no se aconseja borrar nada: se dice lo que se ve y se dice que es
-- pronto para hacerle caso.
...
begin
  ...
  select stats_reset,
         round(extract(epoch from (now() - stats_reset)) / 86400.0, 1)
    into v_desde, v_dias_medidos
  ...
  -- Sólo se aconseja borrar con historial suficiente. Con menos, el dato se
  -- enseña igual pero sin recomendación: cero usos con dos días de medición
  -- quiere decir "todavía no ha hecho falta", no "sobra".
  if jsonb_array_length(v_indices_ociosos) > 0 and coalesce(v_dias_medidos, 0) >= 7 then
    v_avisos := v_avisos || jsonb_build_object(
  ...
  return jsonb_build_object(
    ...
    'indices_sin_usar', jsonb_build_object(
      ...
      'fiable', coalesce(v_dias_medidos, 0) >= 7,
      'nota', case when coalesce(v_dias_medidos, 0) >= 7
                then 'Hay historial suficiente para mirárselo.'
                else 'Pronto para hacerle caso: se lleva midiendo poco tiempo y un índice sin usar puede ser sólo una función del producto que aún no se usa.'
              end
    ),
    'cola',            v_cola,
    'avisos',          v_avisos,

...
export function clienteKms(env = process.env, { crearCliente = (config) => new KMSClient(config) } = {}) {
  if (cliente) return cliente;
  if (!env.AWS_REGION) {
    throw new Error("Falta AWS_REGION para abrir sobres KMS");
  }
  /* No se pasan claves aquí: el SDK resuelve identidad temporal mediante su
     cadena estándar (web identity, credenciales del runtime, perfil local,
     etc.). Así no obligamos a guardar un par IAM de larga duración junto a
     los sobres que protege. */
  cliente = crearCliente({ region: env.AWS_REGION });
  return cliente;
}
...
export async function abrirSobresDeEntorno({ env = process.env, kms = null, nombres = SECRETOS_EN_SOBRE } = {}) {
  ...
  logEvent("info", "kms.sobres_abiertos", { cuantos: cerrados.length, variables: cerrados });

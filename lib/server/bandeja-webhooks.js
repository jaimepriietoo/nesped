...
export async function guardarEvento({ proveedor, tipo = null, eventoId = null, clientId = null, payload = {}, enLinea = false }) {
  ...
  const idExterno = String(eventoId || "").trim();
  if (!idExterno) {
    throw new Error("El webhook no trae un identificador idempotente");
  }
  ...
  if (idExterno) {
    const { data: existente, error: errorBusqueda } = await supabase
      .from("webhook_events").select("id, estado").eq("proveedor", proveedor).eq("evento_id", idExterno).maybeSingle();
  ...
  const { data, error } = await supabase
    .from("webhook_events")
    .insert({ proveedor, tipo, evento_id: idExterno, client_id: clientId, payload })
    .select("id, estado")
    .maybeSingle();

  if (error) {
    if (error.code === "23505" && idExterno) {
  ...
  if (enLinea || webhooksEnLinea()) {
    try {
      ...
      logErrorSeguro("webhook_inbox.inline_failed", err, { provider: proveedor });
    }
  }

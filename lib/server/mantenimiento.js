...
import { rellenarCifrado } from "@/lib/server/cifrado-relleno";
...
export async function pasadaDeMantenimiento() {
  ...
  /* Los sobres de las filas anteriores al cifrado se rellenan por lotes, con
     el tiempo que sobre. Si queda, la pasada se vuelve a encolar. */
  let cifrado = null;
  if (Date.now() < hasta) {
    try {
      cifrado = await rellenarCifrado({ presupuestoMs: Math.max(1000, hasta - Date.now()) });
      quedaTrabajo = quedaTrabajo || Boolean(cifrado?.quedaTrabajo);
    } catch (err) {
      cifrado = { errores: 1, error: String(err?.message || err).slice(0, 200) };
    }
  }

  return { quedaTrabajo, movidas, retencion, cadena, anomalias, cifrado };
}

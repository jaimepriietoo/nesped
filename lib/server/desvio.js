...
export async function desviar(clientId, { telefono = null, actor = "admin" } = {}) {
  ...
  const { error: auditError } = await supabase.from("audit_logs").insert({
    client_id: clientId, entity_type: "desvio", entity_id: clientId,
    action: "desvio_activado", actor,
    changes: { destino_configurado: true, tenia_url_anterior: Boolean(anterior) },
  });
  if (auditError) throw new Error("No se pudo registrar la auditoría del desvío");
...
export async function quitarDesvio(clientId, { actor = "admin" } = {}) {
  ...
  const { error: auditError } = await supabase.from("audit_logs").insert({
    client_id: clientId, entity_type: "desvio", entity_id: clientId,
    action: "desvio_quitado", actor,
    changes: { url_anterior_restaurada: Boolean(empresa.desvio_voice_url_anterior) },
  });
  if (auditError) throw new Error("No se pudo registrar la auditoría del desvío");

...
export async function persistElevenLabsCall({
  ...
  /* Una sola escritura atómica en vez de buscar-y-luego-insertar.
     ...
     rápido. Con el índice único (client_id, call_sid) y un upsert sobre él,
     la segunda entrega actualiza la misma fila en vez de duplicarla. */
  const { error: guardarError } = await supabase
    .from("calls")
    .upsert(baseCallPayload, { onConflict: "client_id,call_sid" });

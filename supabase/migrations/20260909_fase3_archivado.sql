-- Fase 3: archivado de lead_events y audit_logs.
-- Aplicado en producción el 2026-09-09.
--
-- Son las dos tablas que crecen y no paran: una fila por cada cosa que le pasa
-- a un contacto y una por cada acción con consecuencias. Nadie las borra
...
-- tablas —el recorrido de un contacto, el registro de una empresa— pasa por
-- índices que cada mes son más grandes, y esos índices compiten por la memoria
-- con las consultas que sí atienden a alguien.
--
-- No se borra: se mueve. Lo antiguo va a una tabla gemela con un solo índice,
-- que ocupa poco y sigue ahí si alguien pregunta. Consultarla es lento, y está
...
-- Un solo índice por tabla. Lo único que se le pregunta a un archivo es
-- "enséñame lo de esta empresa"; copiar todos los índices de la tabla viva
-- anularía la mitad del motivo de archivar.
...
-- bloqueo largo, hace crecer el WAL y puede tumbar la base de datos durante el
-- rato que dura. Mil filas cada vez tardan más en total y no molestan a nadie.
--
...
begin
  ...
  sacadas as (
    delete from public.lead_events e using viejas v where e.id = v.id returning e.*
  )
...
-- Esta sí borra, y por eso el plazo es largo y separado del de archivar. Un
-- registro de auditoría de hace tres años no sirve para operar y sí puede ser
-- una obligación; el número lo decide quien lleva el cumplimiento.

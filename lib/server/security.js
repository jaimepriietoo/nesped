...
async function callUpstash(pathname) {
  if (!UPSTASH_URL || !UPSTASH_TOKEN) {
    return null;
  }

  const response = await fetch(`${UPSTASH_URL}${pathname}`, {
    method: "POST",
    headers: {
...
export async function consumeRateLimitAsync({
  ...
  const normalizedKeyParts = keyParts.map(normalizeValue);

  if (UPSTASH_URL && UPSTASH_TOKEN) {
    const compositeKey = `nesped:rl:${namespace}:${hashKey(normalizedKeyParts)}`;
    const ttlSeconds = Math.max(1, Math.ceil(windowMs / 1000));

    const increment = await callUpstash(
      `/incr/${encodeURIComponent(compositeKey)}`
    );
    const count = Number(increment?.result || 0);

    if (count <= 1) {
      await callUpstash(
        `/expire/${encodeURIComponent(compositeKey)}/${ttlSeconds}`
      );
    }

    const ttl = await callUpstash(`/ttl/${encodeURIComponent(compositeKey)}`);
    const ttlSecondsResult = Math.max(1, Number(ttl?.result || ttlSeconds));

    return {
      allowed: count <= limit,
      remaining: Math.max(0, limit - count),
      resetAt: Date.now() + ttlSecondsResult * 1000,
      limit,
      strategy: "redis",
    };
  }
...
export async function requireRateLimitAsync(
  req,
  {
    namespace,
    limit,
  ...
  try {
    rateLimit = await consumeRateLimitAsync({
      namespace,
      limit,
      windowMs,
      keyParts: [namespace, getRequestIp(req), ...keyParts],
    });
  } catch {
  ...
  return Response.json(
    ...
    },
    {
      ...
      headers: {
        ...
        "X-RateLimit-Remaining": String(rateLimit.remaining),
        "X-RateLimit-Reset": String(rateLimit.resetAt),
        "X-RateLimit-Strategy": String(rateLimit.strategy || "memory"),
      },
    }
  );
}

...
export const CLIENTS = {
  demo: {
    ...
    type: "IA comercial",
    status: "Activo",
    tagline: "Automatización de llamadas y captación de contactos con IA",
    logoText: "N",
...
export async function ensureDemoWorkspace(supabase, clientId) {
  ...
  if (!existing) {
    const { error: clientError } = await supabase.from("clients").insert({
      ...
      twilio_number: seed.voiceNumber || null,
  ...
  if (existing && seed.voiceNumber) {
    const { error: updateClientError } = await supabase
      .from("clients")
      .update({ twilio_number: seed.voiceNumber })
      .eq("id", seed.id)
    ...
    if (updateClientError) {
      throw new Error(
        updateClientError.message ||
          "No se pudo completar el número de voz del cliente demo"
      );
    }
  }

...
async function apuntar(supabase, { clientId, tipo, leadId, modo, resultado, detalle }) {
  ...
  }).then(({ error }) => { if (error) logErrorSeguro("automations.execution_write_failed", error); });
}
 ...
 */
const ACCIONES = {
  async notificar_departamentos({ supabase, clientId, lead, clasificacion, contextoLlamada }) {
    const r = await notificarLead({ clientId, lead, clasificacion, contextoLlamada, supabase });
  ...
  async email_lead_importante({ supabase, clientId, lead, clasificacion, contextoLlamada, modo, s }) {
    ...
    const r = await notificarLead({ clientId, lead, clasificacion: { ...clasificacion, departamento: "direccion", senales: { ...s, importante: true } }, contextoLlamada, supabase });
  ...
  async pedir_datos_faltantes({ supabase, clientId, lead, modo, canalWhatsapp, consentimiento }) {
    ...
    const texto = `Hola${contacto.nombre ? ` ${lead.nombre}` : ""}, para poder atenderte bien, ¿me dices tu ${faltan.join(" y ")}?`;
 ...
 */
export async function ejecutarAutomatismos({
  clientId, disparo, lead, clasificacion = null, contextoLlamada = null,
  ...
  for (const a of orden) {
    ...
    try {
      ...
      logErrorSeguro("automations.action_failed", err, { automationId: a.id });
    }
  ...
  for (const [clientId, piezas] of porEmpresa) {
    ...
    for (const p of piezas) {
      ...
      try {
        ...
        logErrorSeguro("automations.sweep_failed", err, { automationType: p.tipo });

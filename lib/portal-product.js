...
export function getDefaultPlaybookWorkspace({
  ...
  return {
    ...
    followup:
      "Haz seguimiento corto, útil y con contexto. Evita mensajes largos o robóticos.",
    upsell:
      "Propón subir de plan solo cuando el cliente ya vea valor y el siguiente nivel resuelva una necesidad clara.",
    handoff:
      "Si detectas alta intención, deja el contacto listo para el equipo humano con resumen accionable.",
    notes:
...
export function getPlaybookLibrary({ industry = "", brandName = "Nesped" } = {}) {
  ...
  return [
    ...
    {
      id: "upsell",
      title: "Subida de plan inteligente",
      channel: "email",
      stage: "upsell",
...
export function serializePlaybookWorkspace(workspace = {}) {
  ...
  return [
    ...
    `Seguimiento: ${normalized.followup}.`,
    `Venta adicional: ${normalized.upsell}.`,
    `Traspaso: ${normalized.handoff}.`,
...
export function buildOnboardingChecklist({
  ...
  const items = [
    ...
    },
    {
      ...
      detail: "Activa correos de seguimiento para responsable o equipo.",
      view: "settings",
    },
    {
      id: "pipeline",
      title: "Primer embudo cargado",
      done: leads.length > 0,
      detail: "Sube o genera contactos para activar CRM, puntuación y automatizaciones.",
      view: "pipeline",
    },
  ];
...
export function buildDerivedNotifications({
  ...
  if (metrics.unassignedLeads > 0) {
    notifications.push({
      ...
      title: "Contactos sin responsable",
      body: `Hay ${metrics.unassignedLeads} contactos sin responsable asignado.`,
      area: "pipeline",
      view: "team",
    });
  }
  ...
  function ensureThread(lead) {
    ...
    if (!threadMap.has(lead.id)) {
      threadMap.set(lead.id, {
        id: lead.id,
        leadId: lead.id,
        leadName: lead.nombre || "Contacto sin nombre",
        email: lead.email || "",
        phone: lead.telefono || "",
        owner: lead.owner || "",
  ...
  const integrations = [
    ...
    },
    {
      ...
      detail:
        ...
        "Número de voz, proveedor y flujo de llamadas.",
      view: "voice",
    },
    {
      id: "whatsapp",
    ...
    },
    {
      ...
      detail: "Correos y seguimiento ejecutivo activos.",
      view: "settings",
    },
    {
      id: "playbook",
    ...
    nextActions: [
      ...
      users.length < 2
        ? {
            ...
            detail: "Añade al menos otro usuario con permisos para operar y revisar contactos.",
            view: "team",
          }
        : null,
    ].filter(Boolean),
...
export function buildRoiSnapshot({
  ...
  return {
    ...
    story: [
      ...
      {
        id: "pipeline",
        title: "Valor en el embudo",
        value: pipelineValue,
        detail:
          pipelineValue > 0
            ? "Este valor marca el potencial inmediato si se gestiona bien el embudo actual."
            : "Aún no hay valor estimado suficiente en el embudo.",
      },
      {
        id: "efficiency",
        title: "Ingreso por contacto",
    ...
    focusAreas: [
      ...
      },
      {
        ...
        body: topOwner
          ...
          : "Todavía no hay clasificación por responsable suficiente.",
      },
      {
        id: "lead",
        title: "Contacto más valioso",
        body: topLead
          ? `${topLead.customer_name || "Este contacto"} ya suma ${Math.round(
              toNumber(topLead.total_revenue, 0)
            )}€ en ingresos.`
          : "Todavía no hay contacto con ingreso destacado.",
      },
    ],
  };
}
...
export function buildMessageExperimentSummary({
  ...
  if (totalSent === 0) {
    suggestions.push({
      ...
      body:
        "Todavía no hay volumen suficiente de pruebas. Lanza al menos una variante de WhatsApp y otra de seguimiento para empezar a aprender.",
    });
  }
  ...
  if (winner?.channel === "whatsapp" && winner.reply_rate >= 15) {
    suggestions.push({
      ...
      body:
        "WhatsApp ya está enseñando mejor señal de respuesta. Conviene mover más volumen de seguimiento ahí.",
    });
  }
  ...
  if (suggestions.length < 4 && whatsappTemplates.length > 0) {
    suggestions.push({
      ...
      body: `Usa como base "${whatsappTemplates[0]?.name || "WhatsApp seguimiento"}" y prueba una llamada a la acción más concreta.`,
    });
  }
...
export function buildAdvancedAiInsights({
  ...
  if (hotOpenLeads >= 3) {
    insights.push({
      ...
      body: `Tienes ${hotOpenLeads} contactos muy calientes todavía abiertos. El cuello de botella no es captación, es cierre y seguimiento.`,
      view: "pipeline",
    });
  }
  ...
  if (targetLeads > 0 && totalLeads < targetLeads) {
    insights.push({
      ...
      category: "growth",
      priority: 88,
      title: "La meta de contactos aún está por debajo",
      body: `La cuenta va en ${totalLeads}/${targetLeads} contactos frente a la meta configurada. Conviene reforzar adquisición o automatización temprana.`,
      view: "onboarding",
    });
  }
  ...
  if (targetConversion > 0 && winRate < targetConversion) {
    insights.push({
      ...
      body: `La meta es ${targetConversion}% y ahora mismo la cuenta está en ${winRate}%. Hay margen claro en el cierre, los guiones y el momento de contactar.`,
      view: "playbooks",
    });
  }
  ...
  if (experiments?.summary?.totalSent > 0 && experiments.summary.replyRate < 12) {
    insights.push({
      ...
      body: `Las pruebas activas ya tienen volumen, pero la respuesta sigue en ${experiments.summary.replyRate}%. Toca simplificar el texto y la llamada a la acción.`,
      view: "experiments",
    });
  ...
  if (unassignedLeads > 0) {
    insights.push({
      ...
      category: "ops",
      priority: 76,
      title: "Hay contactos sin responsable",
      body: `${unassignedLeads} contactos siguen sin responsable claro. Eso suele traducirse en seguimiento irregular y pérdida de velocidad comercial.`,
      view: "team",
    });
  }
  ...
  if (topProduct && topProduct.share >= 60) {
    insights.push({
      ...
      body: `${topProduct.name || topProduct.tier} concentra ${topProduct.share}% de los ingresos trazados. Tiene sentido doblar esfuerzos ahí y diseñar la venta adicional alrededor.`,
      view: "strategy",
    });
  }
  ...
  if (totalRevenue > 0 && !client?.stripe_customer_id) {
    insights.push({
      ...
      category: "billing",
      priority: 65,
      title: "El retorno ya existe; la capa de facturación debe quedar perfecta",
      body:
        "Ya hay ingresos trazados y merece la pena dejar la facturación, el portal y las subidas de plan totalmente cerrados para no perder expansión.",
      view: "billing",
    });
  }
...
export function buildStrategySnapshot({
  ...
  const benchmarkCards = [
    {
      ...
      detail:
        targetLeads > 0
          ? `La cuenta va en ${totalLeads}/${targetLeads} contactos frente a su objetivo actual.`
    ...
    },
    {
      ...
      detail:
        targetConversion > 0
          ...
          : "Configura la meta de conversión para valorar la eficiencia real del embudo.",
    },
    {
      id: "ownership",
      label: "Cobertura de responsable",
      value: `${assignedRate}%`,
      target: "100%",
      ...
      detail:
        assignedRate >= 95
          ? "Casi todo el embudo tiene responsable claro."
          : "Todavía hay oportunidades sin responsable fijo, lo que frena seguimiento y responsabilidad.",
    },
    {
      id: "experiments",
  ...
  return {
    summaryCards: [
      {
        id: "revenue",
        label: "Ingresos trazados",
        value: Math.round(totalRevenue),
        suffix: "EUR",
        detail:
          totalRevenue > 0
            ? "Ingresos ya atribuidos a la cuenta."
            : "Aún no hay ingresos trazados en eventos de pago.",
      },
      {
        id: "signals",
        label: "Análisis prioritarios",
      ...
      },
      {
        ...
        detail: topProduct
          ? `${topProduct.share}% de los ingresos actuales llega desde aquí.`
...
export function buildBrandLabWorkspace({
  ...
  const integrations = [
    {
      ...
      detail: domain
        ...
        : "Todavía no hay dominio conectado para marca blanca.",
    },
    {
      id: "voice",
      name: "Telefonía",
    ...
    },
    {
      ...
      detail:
        settings?.daily_report_email || settings?.weekly_report_email
          ? "La cuenta ya envía informes automáticos."
          : "Configura informes diarios o semanales para dejar la capa ejecutiva lista.",
    },
    {
      id: "webhook",
  ...
  const suggestions = [
    !client?.logo_text
      ? {
          ...
          body:
            "Añade texto corto o iniciales de marca para que la marca blanca tenga identidad propia incluso sin logo remoto.",
        }
      : null,
    !domain
      ? {
    ...
    !client?.webhook
      ? {
          ...
          body:
            "Conecta un webhook saliente o integración para que la marca blanca no viva aislado.",
        }
      : null,
  ].filter(Boolean);
  ...
  return {
    preview: {
      ...
      tagline:
        client?.tagline || "Centro de ingresos premium con voz, CRM y automatización.",
...
export function buildAccessCenterSnapshot({
  ...
  if (owners.length === 0) {
    recommendations.push({
      id: "owner-missing",
      title: "Falta un responsable claro",
      body:
  ...
  if (
    ...
    recommendations.push({
      ...
      body:
        "Los roles de propietario y administrador requieren verificación reforzada. Configura RESEND_API_KEY para entregar los códigos por correo en producción.",
    });
  }
...
export function buildApiHubWorkspace({
  ...
  const endpoints = [
    ...
    },
    {
      ...
      description: "Destino para eventos salientes de Nesped hacia el conjunto de herramientas del cliente.",
    },
  ];
  ...
  const eventCatalog = [
    {
      id: "lead_captured",
      title: "lead_captured",
      body: `Evento saliente cuando ${brandName} captura o sincroniza un contacto nuevo.`,
      payload: {
        event: "lead_captured",
        client_id: client?.id || "",
        lead: {
    ...
    },
    {
      ...
      body: "Confirmación de cobro con importe, producto y sesión de pago.",
      payload: {
  ...
  const integrations = [
    ...
    },
    {
      ...
      detail:
        settings?.daily_report_email || settings?.weekly_report_email
          ? "Informes activos para los responsables."
          : "Activa los informes para cerrar la capa ejecutiva.",
    },
  ];
  ...
  return {
    ...
    recipes: [
      {
        id: "crm",
        title: "Enviar contactos al CRM externo",
        body:
          "Usa el webhook de salida para reenviar contacto, puntuación, responsable y siguiente acción al CRM o a la herramienta de análisis del cliente.",
      },
      {
        id: "billing",
      ...
      },
      {
        ...
        body:
          "Los eventos de voz, pagos y captación de contactos permiten avisos internos en Slack, correo o herramientas sin código.",
      },
    ],
  };
}
...
export function buildControlTowerSnapshot({
  ...
  const workflows = [
    {
      id: "funnel",
      name: "Embudo automático",
      eligible: openLeads.length,
      status:
        openLeads.length >= 25 ? "warning" : openLeads.length > 0 ? "healthy" : "low",
      detail:
        openLeads.length > 0
          ? `${openLeads.length} contactos siguen activos en el embudo y pueden beneficiarse de automatización.`
          : "No hay contactos abiertos ahora mismo.",
    ...
    },
    {
      ...
      detail:
        reactivationCandidates.length > 0
          ? `${reactivationCandidates.length} contactos están fríos y sin seguimiento reciente.`
    ...
    },
    {
      ...
      detail:
        hotVoiceCandidates.length > 0
          ? `${hotVoiceCandidates.length} contactos calientes aún no han recibido una llamada reciente.`
    ...
    },
    {
      ...
      detail:
        nextActionGap.length > 0
          ? `${nextActionGap.length} contactos siguen sin siguiente acción clara.`
    ...
    },
    {
      ...
      detail:
        pendingInvoices > 0
          ...
          : "La capa de facturación no muestra incidencias de cobro ahora mismo.",
      actionId: null,
      view: "billing",
    },
    {
  ...
  return {
    ...
    watchlist: [
      ...
      {
        id: "pipeline",
        title: "Contactos estancados",
        body:
          stalledLeads.length > 0
            ? `${stalledLeads.length} contactos llevan más de 15 días sin movimiento.`
            : "No hay estancamiento grave en el embudo.",
        level: stalledLeads.length > 5 ? "warning" : "healthy",
      },
      {
        id: "voice",
        title: "Control de calidad de voz",
        body:
...
export function buildGrowthWorkspace({
  ...
  const levers = [
    {
      id: "followup",
      title: "Seguimiento de contactos calientes",
      value: noFollowup.length,
      level: noFollowup.length >= 5 ? "warning" : "healthy",
      body:
        noFollowup.length > 0
          ? `${noFollowup.length} contactos con puntuación alta aún no tienen cadencia de seguimiento cerrada.`
          : "La capa de seguimiento sobre contactos calientes está bastante sana.",
      view: "leads",
    },
    {
      id: "voice",
      ...
      body:
        totalCalls > 0
          ? `La conversión llamada→contacto está en ${callCaptureRate}%.`
          : "Todavía no hay suficiente actividad de voz para medir cobertura.",
      view: "voice",
    },
    {
      id: "upsell",
      title: "Ingresos por ampliación",
    ...
    },
    {
      ...
      body:
        unassignedLeads.length > 0
          ? `${unassignedLeads.length} contactos abiertos siguen sin owner claro.`
  ...
  const campaigns = [
    {
      id: "reactivation",
      title: "Reactivar embudo templado",
      audience: `${openLeads.filter((lead) => toNumber(lead.score, 0) >= 50).length} contactos`,
      body:
        "Lanza una reactivación corta por WhatsApp y voz sobre contactos con puntuación media-alta pero sin movimiento reciente.",
      view: "automations",
    },
    {
      id: "proof",
      title: "Cerrar con prueba social",
      audience: `${hotLeads.length} contactos calientes`,
      body:
    ...
    },
    {
      ...
      body:
        "Usa ROI y facturación para empujar ampliación de plan o producto cuando el cliente ya está viendo retorno claro.",
      view: "roi",
    },
  ];
  ...
  return {
    ...
    scoreboard: [
      ...
      },
      {
        ...
        detail: "Valor abierto en contactos no cerrados.",
      },
      {
        id: "goal",
        label: "Meta de contactos",
        value: `${monthlyLeadProgress}%`,
        detail: `${leads.length}/${monthlyTargetLeads || 0} contactos frente a la meta actual.`,
      },
      {
        id: "conversion",
...
export function buildEnterpriseWorkspace({
  ...
  const controls = [
    ...
    {
      id: "domain",
      title: "Dominio y marca blanca",
      status: domainReady ? "healthy" : "warning",
      detail: domainReady
        ? `Dominio activo: ${client.custom_domain}`
        : "Falta un dominio propio para cerrar la capa de marca blanca.",
    },
    {
      id: "webhook",
      ...
      detail: webhookReady
        ...
        : "Conviene conectar un webhook o API externa para operar como plataforma de empresa.",
    },
    {
      id: "reporting",
      ...
      detail: reportingReady
        ? "La capa de informes diarios o semanales está activa."
        : "Activa los informes para los responsables y los equipos externos.",
    },
  ];
  ...
  return {
    ...
    rollout: [
      {
        id: "brandlab",
        title: "Cerrar marca blanca",
        body: domainReady
          ? "La base de marca blanca está activa. El siguiente paso es pulir la imagen de marca y el acceso."
          : "Conecta dominio propio y remata la experiencia de marca blanca.",
        view: "brandlab",
      },
      {
        id: "access",
        title: "Blindar acceso y gobierno",
        body: "Revisa roles, responsables y flujos de recuperación para operar como cuenta de empresa.",
        view: "access",
      },
      {
        id: "api",
        title: "Conectar herramientas externas",
...
export function buildCopilotWorkspace({
  ...
  const nextMoves = [
    ...hotLeads.slice(0, 4).map((lead) => ({
      id: `hot:${lead.id}`,
      title: `Mover ya a ${lead.nombre || "contacto caliente"}`,
      body: `Puntuación ${lead.score || 0}, estado ${getStatusSentence(lead.status)} y valor estimado ${fmtCurrencyLike(lead.valor_estimado || 0)}.`,
      priority: "alta",
      view: "leads",
      leadId: lead.id,
    })),
    ...staleLeads.slice(0, 3).map((lead) => ({
      id: `stale:${lead.id}`,
      title: `Reactivar a ${lead.nombre || "contacto estancado"}`,
    ...
    ...noOwnerLeads.slice(0, 2).map((lead) => ({
      id: `owner:${lead.id}`,
      title: `Asignar responsable a ${lead.nombre || "contacto sin responsable"}`,
      body: "Sin responsable claro se pierde velocidad comercial y responsabilidad.",
      priority: "alta",
      view: "team",
      leadId: lead.id,
    })),
  ...
  const scripts = [
    ...
    {
      id: "handoff",
      title: "Paso a una persona",
      body: "Tengo ya contexto suficiente y la prioridad es alta. Asigna responsable, confirma disponibilidad y entra por llamada o WhatsApp con una única llamada a la acción.",
      channel: "ops",
    },
  ];
    ...
    },
    {
      ...
      body:
        noNextActionLeads.length > 0
          ? `${noNextActionLeads.length} contactos siguen sin siguiente acción explícita.`
          : "Los contactos abiertos ya tienen una línea clara de movimiento.",
  ...
  return {
    ...
    briefing:
      nextMoves.length > 0
        ? `Hoy el foco debería estar en ${nextMoves[0].title.toLowerCase()} y en cerrar la fricción operativa de los contactos más calientes.`
        : `Hoy el sistema está estable. Aprovecha para subir calidad de mensajes, control de calidad de voz y expansión de cuenta.`,
    nextMoves,
    scripts,
    watchouts,
    wins,

...
import { peticionExternaSegura } from "@/lib/server/url-segura";

/**
 * Las grabaciones, en casa.
 *
 * Hasta ahora `recording_url` guardaba la dirección que da el proveedor, y el portal
 ...
 * mientras el audio seguía en casa del proveedor. El aviso legal dice que se
 * borraba.
 *
...
export async function copiarGrabacion({ callSid, clientId }) {
  ...
  /* Las llamadas de ElevenLabs no traen URL de grabación: el audio se pide
     a su API por el id de la conversación (call_sid = conv_…), con la clave.
     Las de otros proveedores traen recording_url y se bajan sin más. */
  const deElevenLabs = !llamada.recording_url && /^conv_/.test(String(llamada.call_sid || ""));
  if (!llamada.recording_url && !deElevenLabs) return { copiada: false, motivo: "no hay grabación que copiar" };

  const audio = await conCortacircuitos({ proveedor: "elevenlabs", quienEspera: "nadie" }, async () => {
    const respuesta = deElevenLabs
      ? await fetch(`https://api.elevenlabs.io/v1/convai/conversations/${encodeURIComponent(llamada.call_sid)}/audio`, {
          headers: { "xi-api-key": String(process.env.ELEVENLABS_API_KEY || "") },
          signal: AbortSignal.timeout(60_000),
        })
      : await peticionExternaSegura(llamada.recording_url, {
    ...
    /* Un 404 quiere decir que la grabación ya no está. Reintentarlo cinco
       veces no la va a resucitar, y no dice nada de la salud del proveedor. */
...
export async function direccionParaEscuchar(rutaGuardada) {
  ...
  /* `nesped_app` no recibe ningún permiso sobre storage.*. La ruta ya ha
     comprobado la pertenencia de la llamada con RLS; sólo la firma del objeto
     usa la credencial interna de Storage. */
  const { data, error } = await getSupabaseAdministrativo()
...
export async function borrarGrabacion(rutaGuardada) {
  ...
  const { error } = await getSupabaseAdministrativo().storage.from(DEPOSITO).remove([rutaGuardada]);

...
export async function requirePortalRoleOrInternal(req, allowedRoles = ["owner", "admin", "manager"], options = {}) {
  if (isAuthorizedInternalRequest(req)) return { ok: true, type: "internal", ctx: null };
  const originError = requireSameOrigin(req, options.sameOriginMessage);
  if (originError) return { ok: false, response: originError };
  const ctx = await getPortalContext();
  if (!ctx.ok) return { ok: false, response: Response.json({ success: false, message: "No autorizado" }, { status: 401 }) };
  if (allowedRoles.length && !hasRole(ctx.role, allowedRoles)) {
    return { ok: false, response: Response.json({ success: false, message: "Sin permisos" }, { status: 403 }) };
  }
  return { ok: true, type: "portal", ctx };
}

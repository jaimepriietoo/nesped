...
import { productoPorId } from "@/lib/server/datos";
...
export async function getClientBillingState(clientId) {
  ...
  /* Desde Postgres. Antes salía de Prisma contra un SQLite que no se
     despliega, así que en producción esto siempre devolvía null: nadie tenía
     nunca una suscripción "gestionada" a ojos del código. */
  const { data: activeSubscription } = await getSupabase()
    .from("subscriptions")
    .select("*")
...
export async function resolveCheckoutProduct({ plan, productId }) {
  if (productId) {
    const product = await productoPorId(productId);

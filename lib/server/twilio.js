 ...
 */
async function pedirATwilio(ruta, campos, { quienEspera = "nadie", metodo = "POST" } = {}) {
  ...
  /* Un GET lleva los campos en la URL; un POST, en el cuerpo. */
  const consulta = metodo === "GET" && campos ? `?${new URLSearchParams(campos)}` : "";

  return conCortacircuitos({ proveedor: "twilio", quienEspera }, async () => {
    const respuesta = await fetch(
      `https://api.twilio.com/2010-04-01/Accounts/${encodeURIComponent(accountSid)}${ruta}${consulta}`,
      {
        method: metodo,
        headers: {
        ...
        body: metodo === "GET" ? undefined : new URLSearchParams(campos).toString(),
...
/**
 * El número de Twilio de una empresa: su SID y a dónde manda hoy las llamadas.
 * Hace falta para desviarlo y para volver a dejarlo como estaba.
 */
export async function numeroDeTwilio(telefono, { quienEspera = "persona" } = {}) {
  const json = await pedirATwilio("/IncomingPhoneNumbers.json", { PhoneNumber: telefono, PageSize: "1" }, { quienEspera, metodo: "GET" });
  const n = (json?.incoming_phone_numbers || [])[0];
  if (!n) return null;
  return { sid: n.sid, telefono: n.phone_number, voiceUrl: n.voice_url || "", voiceMethod: n.voice_method || "POST" };
}

/** Cambia a dónde manda las llamadas un número. */
export async function apuntarNumeroDeTwilio(sid, { voiceUrl, voiceMethod = "POST" }, { quienEspera = "persona" } = {}) {
  return pedirATwilio(`/IncomingPhoneNumbers/${encodeURIComponent(sid)}.json`, { VoiceUrl: voiceUrl, VoiceMethod: voiceMethod }, { quienEspera });
}

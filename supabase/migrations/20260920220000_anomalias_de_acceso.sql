-- Lo que hace falta guardar para detectar comportamiento raro.
--
-- Dos tablas pequeñas, cerradas al Data API como el resto:
--   - paises_de_acceso: desde qué países ha entrado cada persona. Un país
--     nuevo en una cuenta que ya tenía otros exige el segundo factor aunque
--     el rol no lo pida, y avisa.
--   - accesos_denegados: cada 403 del portal, con quién y qué ruta. Un
--     usuario de sólo lectura que pide rutas de escritura una y otra vez es
--     alguien probando, y hay que enterarse.
-- Las reglas viven en lib/server/anomalias.js y se evalúan a diario.
...
-- Los 403 de hace más de treinta días ya no dicen nada.

...
async function manejarGET() {
  ...
  try {
    ...
    console.error("[portal/automatismos]", err?.message || err);
...
async function manejarPATCH(req) {
  ...
  const leido = validar(AutomatismoCambio, await req.json().catch(() => ({})));
  if (leido.respuesta) return leido.respuesta;
  try {
    const { tipo, ...cambios } = leido.datos;
    const fila = await fijarAutomatismo(ctx.clientId, tipo, cambios, ctx.datos);
    await ctx.datos.from("audit_logs").insert({ client_id: ctx.clientId, entity_type: "automatismo", entity_id: tipo, action: "automatismo_cambiado", actor: ctx.userEmail, changes: cambios });

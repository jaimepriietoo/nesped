 ...
 */
export async function clasificarLead({
  clientId, leadId, textoExtra = [], soloTextoExtra = false, supabase = getSupabase(),
}) {
  ...
  /* En una llamada, la clasificación pertenece a esa conversación. Mezclar
     necesidad/notas históricas puede mandar el correo nuevo al departamento
     antiguo y, peor, volver a introducir datos viejos en el aviso. */
  const texto = soloTextoExtra
    ? textoDelLead(null, textoExtra)
    : textoDelLead(lead, textoExtra);
  if (!texto) {

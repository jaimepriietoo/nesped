...
async function getLeadOrThrow({ supabase, leadId, clientId }) {
  ...
  if (error || !lead) {
    throw new Error(error?.message || "Contacto no encontrado");
  }
...
`;
  ...
  try {
    ...
    const response = await conRegistroIA({ clientId: lead.client_id, uso: "siguiente-accion", modelo: "gpt-5-mini", promptVersion: "nba-v1" }, () =>
    ...
    logErrorSeguro("next_best_action.llm_fallback", error);
    return fallback;
  }
}
...
export async function saveNextBestAction({
  ...
  if (updateError || !updatedLead) {
    throw new Error(updateError?.message || "No se pudo actualizar el contacto");
  }
...
export async function executeNextBestAction({
  ...
  if (!action || action === "wait") {
    return {
      success: false,
      message: "Este contacto no tiene una acción ejecutable.",
    };
  }
  ...
  if (action === "sms") {
    ...
    await Promise.all([
      createAuditLog(supabase, {
        ...
        meta: { message, sid: messageId, messageId, actor, provider: "twilio" },
      }),
      createLeadEvent(supabase, {
        clientId,
        leadId,
    ...
    return { success: true, mode: "sms", sid: messageId, messageId };
  }
  ...
  if (action === "whatsapp") {
    ...
    await supabase
      ...
      .update({
        last_contact_at: nowIso,
        last_contacted_at: nowIso,
        ultima_accion: "WhatsApp enviado desde acción recomendada",
      })
      .eq("id", lead.id)
      .eq("client_id", clientId);
    ...
    return { success: true, mode: "whatsapp", sid: messageId, messageId };
  }

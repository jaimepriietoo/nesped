-- Sesiones con nombre y apellidos.
--
-- Hasta ahora una sesión era sólo un token firmado: no se sabía cuántas
-- había abiertas ni desde dónde, y la única palanca era "cerrar todas". Con
-- esta tabla cada inicio de sesión deja una fila, cada petición la toca
-- (como mucho una vez por minuto), y se puede cerrar una sola.
--
-- No se guarda la IP entera ni el User-Agent entero: un prefijo /24 y el
-- nombre del navegador, lo justo para que la persona reconozca "mi móvil".
-- Aditiva: tabla nueva, cerrada al Data API, con política para nesped_app
-- igual que el resto de tablas de empresa.
...
-- (el token caduca a los siete). Se limpian con el resto de la seguridad.

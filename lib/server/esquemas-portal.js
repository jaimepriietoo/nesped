...
export const Departamento = z.object({
  ...
  areas: z.array(texto(40).min(1)).max(10).optional().default([]),
...
export const CerrarSesion = z.object({
  id: z.uuid("Identificador de sesión no válido"),
});

const RespuestaWebAuthn = z.object({
  id: texto(1024).min(1),
  rawId: texto(1024).min(1),
  type: z.literal("public-key"),
  response: z.record(z.string(), z.unknown()),
  clientExtensionResults: z.record(z.string(), z.unknown()).optional().default({}),
  authenticatorAttachment: texto(40).optional(),
}).passthrough();

export const GestionPasskeys = z.discriminatedUnion("action", [
  z.object({ action: z.literal("start") }),
  z.object({ action: z.literal("finish"), response: RespuestaWebAuthn, nombre: texto(80).optional().default("") }),
  z.object({ action: z.literal("delete"), id: z.uuid(), code: CodigoTotpORecuperacion.optional() }),
]);

export const AccesoConPasskey = z.object({
  response: RespuestaWebAuthn,
});

export const GestionConocimiento = z.discriminatedUnion("action", [
  z.object({
    action: z.literal("add"),
    texto: texto(2000).min(1, "Escribe qué tiene que saber la IA"),
    vigente_hasta: texto(64).refine((v) => v === "" || Number.isFinite(Date.parse(v)), "Fecha no válida").optional().default(""),
  }),
  z.object({ action: z.literal("remove"), id: z.uuid() }),
  z.object({ action: z.literal("pin"), pin: z.string().regex(/^\d{4,6}$/, "El PIN son entre 4 y 6 cifras") }),
  z.object({ action: z.literal("pin_off") }),
]);

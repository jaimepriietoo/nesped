...
export async function restablecerConToken({ token, password, supabase = getSupabase(), revocar = revocarSesionesDe }) {
  const { data: fila } = await supabase.from("restablecer_password").select("*").eq("token_hash", hashDe(token || "")).maybeSingle();
  if (!fila || fila.used_at || new Date(fila.expires_at).getTime() < Date.now()) {
    throw Object.assign(new Error("El enlace no es válido o ha caducado. Pide uno nuevo."), { status: 400 });
  }
  const revision = validarPassword(password, { email: fila.email });
  ...
  if (error) throw new Error(error.message);
  await supabase.from("restablecer_password").update({ used_at: new Date().toISOString() }).eq("id", fila.id);

  ...
  try {
    ...
    for (const { destinatario, motivo } of elegidos) {
      ...
      try {
        ...
        console.error("[destinatarios] no se pudo avisar:", error, contextoActual().request_id || "");
      }
    }
  } catch (err) {
    console.error("[destinatarios] notificarLead:", err?.message || err);
  }
  return resumen;
}
...
export async function notificarLlamada({ clientId, callSid, supabase = getSupabase() }) {
  ...
  try {
    ...
    console.error("[destinatarios] notificarLlamada:", err?.message || err);
  }
  return resumen;
}

...
export function decideActionType(lead) {
  ...
  if (status === "won" || status === "lost") {
    return {
      action: "wait",
      priority: "none",
      reason: "El contacto ya está cerrado o descartado.",
    };
  }
  ...
  if (status === "qualified" && canCall && (score >= 75 || prob >= 70) && hours >= 12) {
    return {
      action: "call",
      priority: "high",
      reason: "Contacto cualificado y caliente. Conviene llamada directa de cierre.",
    };
  }
  ...
  if (status === "new" && canCall && score >= 85 && interest === "alto" && hours >= 1) {
    return {
      action: "call",
      priority: "high",
      reason: "Contacto nuevo, muy caliente y con alta intención. Mejor llamada inmediata.",
    };
  }
  ...
  if (status === "contacted" && canWhatsapp && interest === "alto" && hours >= 24) {
    return {
      action: "whatsapp",
      priority: "high",
      reason: "Contacto con interés alto sin seguimiento reciente. WhatsApp es la vía más rápida.",
    };
  }
  ...
  if (status === "new" && canWhatsapp && score >= 55 && hours >= 2) {
    return {
      action: "whatsapp",
      priority: "medium",
      reason: "Contacto prometedor. WhatsApp permite activar conversación con baja fricción.",
    };
  }
  ...
  if (hours < 6) {
    return {
      action: "wait",
      priority: "low",
      reason: "El contacto es reciente. Conviene no insistir todavía.",
    };
  }

...
import { condicionOr, modoCifrado } from "@/lib/server/cifrado-datos";
...
export async function ultimaLlamadaDeLead({ client_id, lead_id, phone }) {
  if (!client_id) throw new Error("Última llamada: falta la empresa");
  let q = getSupabase().from("calls").select("id, lead_id, phone, status, result, created_at");
  const partes = [];
  if (esUuid(lead_id)) partes.push(`lead_id.eq.${lead_id}`);
  if (phone) partes.push(condicionOr(
    "calls", ["phone", "from_number"], phone, modoCifrado(), process.env, client_id,
  ));
  if (partes.length === 0) return null;
  q = q.eq("client_id", client_id).or(partes.join(","));
  const { data, error } = await q.order("created_at", { ascending: false }).limit(1).maybeSingle();
...
export async function actualizarLlamada(id, cambios, clientId) {
  if (!clientId) throw new Error("Actualizar llamada: falta la empresa");
  const { error } = await getSupabase().from("calls").update(cambios)
...
export async function llamadasDeVoz({ client_id, cuantos = 50, conTranscripcion = false }) {
  ...
  if (conTranscripcion) {
    ...
    q = q.not(modo === "cifrado" || modo === "solo" ? "transcript_cifrado" : "transcript", "is", null);
  }
  const { data, error } = await q.order("created_at", { ascending: false }).limit(cuantos);

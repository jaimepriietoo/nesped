...
export function initClientSentry() {
  ...
  Sentry.init({
    ...
    /* Sólo en producción. Con `enabled: true` a secas, Sentry recibía los
       errores de compilación del servidor de desarrollo —"./v3.css",
       "./app/page.js:6:1"— mezclados con los de verdad, y con cero usuarios
       detrás. Un panel de errores con ruido de desarrollo es un panel que
       se deja de mirar. */
    enabled: process.env.NODE_ENV === "production",
    environment:
      String(process.env.NEXT_PUBLIC_SENTRY_ENVIRONMENT || "").trim() ||
      String(process.env.NEXT_PUBLIC_VERCEL_ENV || "").trim() ||

...
function secreto() {
  return process.env.NESPED_SESSION_SECRET || "";
}
...
export async function leerSesionFirmada(req) {
  try {
    ...
    const clave = secreto();
    if (clave.length < 32) return null;
    ...
    if (datos.purpose !== "session" || !datos.email || !datos.clientId) return null;
    if (!Number.isFinite(emitido) || !Number.isFinite(caduca)) return null;
    if (!emitido || emitido > ahora + MARGEN_RELOJ_MS) return null;
    if (caduca <= ahora || caduca > emitido + 7 * 24 * 60 * 60 * 1000) return null;

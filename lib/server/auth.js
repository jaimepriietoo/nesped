...
import { ampliarContexto, fijarClienteDeDatos } from "@/lib/server/contexto.mjs";
import { clienteDePortal } from "@/lib/server/supabase-empresa";
...
export async function setAuthCookies({
   ...
   */
  /* Cada inicio de sesión deja una fila y el token lleva su id y la huella
     del navegador que la abrió. Ver lib/server/sesiones.js. */
  const huella = await huellaDePeticion();
  const sid = await abrirSesion({ email: normalizedEmail, clientId, huella });
  const payload = {
    ...
    sid,
    huella: { ua: huella.ua, ip: huella.ip },
    issuedAt: Date.now(),
...
export async function revocarSesionesDe(email) {
  ...
  if (!error && data !== null) {
    /* Las filas también, para que la lista de sesiones no las siga enseñando. */
    const { data: filas } = await supabase.from("users").select("client_id").eq("email", normalizado);
    for (const fila of filas || []) {
      await cerrarTodasLasSesiones({ email: normalizado, clientId: fila.client_id }).catch(() => {});
    }
  }
...
export async function getAuthenticatedUserContext() {
  ...
  /* Huella e inactividad. Un token usado desde otro navegador (o, para owner
     y admin, desde otra red) no vale; y la fila de la sesión tiene que
     existir, no estar cerrada y, para owner y admin, haberse usado en los
     últimos quince minutos. */
  const role = user.role || "viewer";
  if (!huellaCoincide({ token: session.huella, actual: await huellaDePeticion(), role })) {
    return { ok: false, message: "Tu sesión ha caducado. Vuelve a entrar." };
  }
  const motivo = await comprobarSesion({ sid: session.sid, email, clientId: session.clientId, role });
...
export async function getPortalSessionContext() {
  ...
  /* El cliente de base de datos del portal sale ya acotado a la empresa de
     ...
     `sinFiltroDeEmpresa` sólo elimina el filtro redundante de JavaScript: con
     RLS activo sigue sin poder salir de la empresa. Los trabajos que cruzan
     empresas viven fuera del portal y piden el cliente administrativo por su
     nombre. */
  const acceso = clienteDePortal({
    clientId: ctx.user.client_id,
    clienteAnterior: ctx.supabase,
  });
  if (acceso.rlsActivo) fijarClienteDeDatos(acceso.cliente);
  ampliarContexto({
    client_id: ctx.user.client_id,
    rls_portal: acceso.rlsActivo ? "activo" : acceso.modo,
  });
  const supabase = datosDeLaEmpresa(acceso.cliente, ctx.user.client_id);
  ...
  return {
    ...
    permissions,
    currentUser,
    /* Sólo el id de la sesión: para marcar "ésta" en la lista y cerrarla. */
    session: { sid: ctx.session?.sid || null },
    supabase,
    datos: supabase,
  };
}

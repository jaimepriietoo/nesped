...
import { hasTwilioSmsConfig, enviarSms } from "@/lib/server/twilio";
...
export function buildTwoFactorEmail({ email, code, clientName = "", role = "" }) {
  ...
  const safeBrand = String(brand).replace(/[&<>"']/g, char => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[char]);
    ...
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #08111f; color: #f5f7fb; padding: 32px;">
        <div style="max-width: 520px; margin: 0 auto; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 24px; padding: 28px;">
          ...
          <h1 style="font-size: 28px; margin: 0 0 12px;">Tu código para entrar en ${safeBrand}</h1>
...
async function enviarPorEmail({ email, code, clientName = "", role = "" }) {
  ...
  let providerId = "";
  try {
    /* "persona": hay alguien esperando este código para entrar. Se intenta
       aunque el cortacircuitos de Resend esté abierto. Una petición no es una
       tormenta, y dejar a alguien fuera de su cuenta porque el proveedor va
       lento es peor que hacer una llamada de más. */
    providerId = await enviarCorreo({
      quienEspera: "persona",
      ...message,
    });
  } catch (err) {
    logEvent("error", "auth.2fa_code_failed", {
      ...
      provider: "resend",
      providerError: {
        name: err?.detalle?.name || err?.name || "ResendError",
        message: err?.message || "Error enviando el código",
      },
    });
...
export async function sendTwoFactorCode({
  ...
  try {
    ...
    const puedeSms = hasTwilioSmsConfig() && esTelefonoValido(telefono);

    if (!puedeSms) {
      logEvent("error", "auth.2fa_sin_via_alternativa", {
        email,
        role,
        motivo: errorEmail?.message || "correo falló",
        tieneSms: hasTwilioSmsConfig(),
    ...
    try {
      /* Iba como `message` y sendTelnyxSms espera `text`. El texto llegaba
         vacío y saltaba la comprobación de "faltan datos": este respaldo no
         podía enviar nada, nunca, aunque todo lo demás estuviera puesto.
         Es la tercera razón por la que no funcionaba, después de que ningún
         usuario tenga móvil guardado y de que no haya número desde el que
         enviar. Un plan B tiene que estar probado o no es un plan.

         "persona": hay alguien esperando este código para entrar. Se intenta
         aunque el circuito de Telnyx esté abierto. */
      await enviarSms({
        to: telefono,
        text: `${code} es tu código de acceso a ${clientName || "Nesped"}. Caduca en 10 minutos.`,
        quienEspera: "persona",
      });
      ...
      logEvent("warn", "auth.2fa_respaldo_sms", {
        email,
        role,
        motivoEmail: errorEmail?.message || "correo falló",
      });
      ...
      throw new Error(
        "No hemos podido enviarte el código ni por correo ni por SMS. " +

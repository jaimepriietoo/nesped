...
import {
  crearEventoLead, eventosDeLead, eventosPorTipo, guardarMemoriaLead,
  ultimaLlamadaDeLead, crearLlamada, actualizarLlamada,
} from "@/lib/server/datos";
import { enviarWhatsApp } from "@/lib/server/twilio";
...
export async function updateLeadDirect(leadId, changes, clientId, supabase = null) {
  if (!clientId) throw new Error("Actualizar contacto: falta la empresa");
  supabase ||= getSupabase();
  const { error } = await supabase.from("leads").update(changes)
    .eq("id", leadId).eq("client_id", clientId);
  if (error) {
    throw new Error(error.message || "No se pudo actualizar contacto");
  }
}

export async function createReminderDirect(
  leadId,
  title,
  remind_at,
  assigned_to = "",
  ...
  if (!clientId) throw new Error("Crear recordatorio: falta la empresa");
  supabase ||= getSupabase();
...
/* Las dos funciones que usa todo el mundo para escribir sobre un contacto.
   La empresa la resuelven desde el propio contacto si quien llama no la
   pasa: los procesos por lotes recorren contactos de todas las empresas y
   no la tienen a mano. */
export async function updateLeadMemoryDirect(leadId, payload, clientId = null) {
  await guardarMemoriaLead(leadId, payload, clientId);
}

export async function createLeadEventDirect({ lead_id, phone, type, message, client_id = null }) {
  return crearEventoLead({ client_id, lead_id: lead_id || null, phone: phone || null, type, message });
}
...
export async function runOnboardingAutomation() {
  ...
  const paymentEvents = await eventosPorTipo({ type: "payment_completed", cuantos: 100 });
  ...
  for (const payment of paymentEvents) {
    try {
      ...
      const history = await eventosDeLead({
        lead_id: lead.id, phone: normalizePhone(lead.telefono || "") || null, cuantos: 50,
      });
      ...
      await updateLeadDirect(lead.id, {
        ...
        ultima_accion: "Alta automática iniciada tras el pago",
        proxima_accion: "Entrega / activación / alta",
      ...
      logErrorSeguro("automation_service.onboarding_item_failed", err);
...
export async function runFunnelAutomation() {
  ...
  for (const lead of leads) {
    try {
      ...
      const history = await eventosDeLead({ lead_id: leadId, phone, cuantos: 50 });
      ...
      if (status === "new") {
        ...
        if (!alreadyStarted) {
          ...
          await createLeadEventDirect({
            client_id: lead.client_id, lead_id: leadId, phone, type: "funnel_new_contact", message,
          });
          await updateLeadDirect(leadId, {
            status: "contacted",
            next_action: "whatsapp",
            next_action_priority: "media",
            ultima_accion: "Primer contacto automático del embudo",
            proxima_accion: "Esperar respuesta del contacto o continuar seguimiento",
          }, lead.client_id);
      ...
      if (status === "contacted") {
        ...
        if (!hasRecentReply && !alreadyFollowed) {
          ...
          await sendWhatsAppMessage(phone, message);
          await createLeadEventDirect({
            client_id: lead.client_id, lead_id: leadId, phone, type: "funnel_contacted_followup", message,
          });
          await updateLeadDirect(leadId, {
            next_action: "whatsapp",
            next_action_priority: "media",
            ultima_accion: "Seguimiento automático en etapa contacted",
            proxima_accion: "Esperar respuesta o recalificar contacto",
          }, lead.client_id);
      ...
      if (status === "qualified") {
        ...
        if (!alreadyPushed) {
          ...
          await sendWhatsAppMessage(phone, message);
          await createLeadEventDirect({
            client_id: lead.client_id, lead_id: leadId, phone, type: "funnel_qualified_push", message,
          });
          await updateLeadDirect(leadId, {
            ...
            proxima_accion: paymentLink
              ...
              : "Empujar cita o llamada",
          }, lead.client_id);
          processed.push({ leadId, stage: "qualified_push" });
        }
      }
    } catch (err) {
      logErrorSeguro("automation_service.funnel_item_failed", err);
...
export async function runVoiceCallsAutomation() {
  ...
  for (const lead of leads) {
    try {
      ...
      const lastCall = await ultimaLlamadaDeLead({ client_id: lead.client_id, lead_id: leadId, phone });

      if (lastCall && hoursSince(lastCall.created_at) < 24) continue;

      const voiceCall = await crearLlamada({
        client_id: lead.client_id, lead_id: leadId, phone, status: "queued",
      });
      ...
      await actualizarLlamada(voiceCall.id, {
        status: finalStatus,
        result: finalResult,
        summary,
        transcript,
      ...
      logErrorSeguro("automation_service.voice_item_failed", err);
      failed.push({ leadId: lead?.id || null });
    }
  }

import { crearEventoLead, guardarMemoriaLead } from "@/lib/server/datos";
...
async function createPaymentEvent({
  ...
  /* La comprobación de duplicados vive en reclamar_webhook(), arriba del
     todo: si se llega hasta aquí es que el evento es nuevo. */
  return crearEventoLead({
    client_id: clientId || session.metadata?.client_id || null,
    lead_id: leadId || null,
    phone: phone || null,
...
async function updateLeadAfterPayment({
  ...
  const updatePayload = {
    ...
    proxima_accion: "Cliente ya pagó. Pasar a alta o entrega.",
  ...
  await guardarMemoriaLead(leadId, {
    payment_sent: true,
...
async function sinTumbarElPago(que, hacer) {
  try {
    ...
    logErrorSeguro("stripe.post_payment_side_effect_failed", error, { operation: que });
  }
}

export async function reclamarEventoStripe(eventId, { supabase = getSupabase() } = {}) {
  const id = String(eventId || "").trim();
  if (!id) throw new Error("Evento de Stripe sin identificador");

  const { data: primeraVez, error } = await supabase.rpc("reclamar_webhook", {
    p_proveedor: "stripe",
    p_evento_id: id,
  });
  if (error || typeof primeraVez !== "boolean") {
...
export async function processStripeWebhookEvent(event) {
  ...
  /* Un evento sólo se aplica una vez.
     ...
     Si la reclamación falla se devuelve error para que Stripe o la cola
     reintenten. Procesar sin saber si otro intento ya ganó rompería la
     garantía de una sola aplicación. */
  const primeraVez = await reclamarEventoStripe(event?.id, { supabase });
  if (primeraVez === false) {
    logEvent("info", "stripe.event_already_applied");
    return;
  }
  ...
  if (event.type === "checkout.session.completed") {
    ...
    /* ORDEN: primero lo que le da al cliente lo que ha pagado.
       ...
       Antes esto empezaba por createPaymentEvent(), que escribía en Prisma
       sobre un SQLite que no se desplegaba: la excepción subía y el webhook
       devolvía 500 ANTES de activar el plan. Resultado: se cobraba el dinero
       y la cuenta se quedaba en "pendiente" para siempre. Prisma ya no
       existe, pero el orden se queda: es correcto por sí mismo.
    ...
    try {
      ...
      /* Si lo esencial falla, el evento NO cuenta como aplicado: se suelta
         la reclamación para que el reintento —de la cola o de Stripe— lo
         vuelva a intentar de verdad en vez de darlo por hecho. */
      if (event?.id) {
        await supabase.from("webhooks_procesados").delete()
          .match({ proveedor: "stripe", evento_id: event.id })
          .then(({ error: e }) => { if (e) logErrorSeguro("stripe.claim_release_failed", e); });
      }
      throw err;
    }
    ...
    await sinTumbarElPago("actualización del contacto", () =>

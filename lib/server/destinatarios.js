 ...
 */
export async function notificarLead({
  clientId, lead, clasificacion, contextoLlamada = null, supabase = getSupabase(),
}) {
  ...
  try {
    ...
    for (const { destinatario, motivo } of elegidos) {
      const fila = {
        ...
        motivo: contextoLlamada?.callSid ? `llamada:${contextoLlamada.callSid}` : motivo,
      };
      if (contextoLlamada?.callSid) {
        const { data: yaEnviado } = await supabase.from("notificaciones_lead").select("id")
          .eq("client_id", clientId).eq("motivo", fila.motivo)
          .eq("email", destinatario.email).eq("estado", "enviado").limit(1);
        if (yaEnviado?.length) {
          resumen.omitidos += 1;
      ...
      try {
        const { asunto, html } = contextoLlamada?.callSid
          ? correoDeLlamada({
              empresa: nombreEmpresa,
              llamada: contextoLlamada.llamada || { call_sid: contextoLlamada.callSid },
              dicho: contextoLlamada.dicho,
              nombreConocido: contextoLlamada.nombreConocido,
              clasificacion,
              destinatario,
              urlPortal,
            })
          : correoDeLead({ empresa: nombreEmpresa, lead, clasificacion, destinatario, urlPortal });
...
const CAMPOS_DE_LA_LLAMADA = [
  "nombre", "email", "telefono", "ciudad", "direccion",
  "necesidad", "preferencia", "resumen", "notas",
];

/** Une las entregas parciales de la herramienta durante una conversación. */
export function combinarDatosDeLlamada(eventos = []) {
  const unidos = {};
  for (const evento of eventos || []) {
    const meta = evento?.meta && typeof evento.meta === "object" ? evento.meta : {};
    for (const campo of CAMPOS_DE_LA_LLAMADA) {
      const valor = String(meta[campo] || "").trim();
      if (valor) unidos[campo] = valor.slice(0, 2000);
    }
  }
  return unidos;
}

/** El correo de una llamada: quién, cuánto y exclusivamente qué dijo ahora. */
/**
 * El correo lleva SÓLO lo dicho en esta llamada. Aunque la persona ya fuera
 * un contacto con ficha, aquí no se copia nada de la ficha: lo pidió la
 * empresa, para que cada aviso sea lo que esa persona dijo esa vez y no una
 * mezcla con lo de hace un mes. `dicho` son los datos que la IA recogió en
 * la conversación (evento datos_de_llamada). El nombre conocido puede usarse
 * como identidad; la clasificación, si aparece, es la de esta llamada.
 */
export function correoDeLlamada({
  empresa, llamada, dicho = {}, nombreConocido = "", clasificacion = null,
  destinatario, urlPortal,
}) {
  const cuando = llamada.created_at ? new Date(llamada.created_at).toLocaleString("es-ES", { timeZone: "Europe/Madrid" }) : "";
  const asunto = `Llamada en ${empresa}: ${dicho.nombre || nombreConocido || llamada.from_number || "número oculto"} · ${Math.round(Number(llamada.duration_seconds || 0))} s`;
  const filas = [
    ["Cuándo", cuando], ["Desde", llamada.from_number || "número oculto"], ["Duración", `${Math.round(Number(llamada.duration_seconds || 0))} segundos`],
    ["Estado", etiqueta(ESTADO_LLAMADA, llamada.status, "")], ["Resumen de la llamada", llamada.summary],
    ["Nombre conocido", !dicho.nombre ? nombreConocido : ""], ["Ha dicho llamarse", dicho.nombre],
    ["Teléfono que ha dado", dicho.telefono], ["Correo que ha dado", dicho.email],
    ["Localidad que ha dado", dicho.ciudad], ["Dirección que ha dado", dicho.direccion], ["Lo que necesita", dicho.necesidad],
    ["Preferencia", dicho.preferencia], ["Notas de la llamada", dicho.notas],
    ["Derivado a", clasificacion?.nombreDepartamento || clasificacion?.departamento],
    ["Por qué", clasificacion?.motivo],
  ].filter(([, v]) => v);
  ...
  const html = `<div style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;max-width:640px;margin:0 auto;color:#111">
    ...
    <p style="font-size:12px;color:#999;margin-top:22px">Recibes esto porque en ${escapar(empresa)} te asignaron ${destinatario.recibe_todo ? "copia de todas las llamadas" : "este departamento"}. Hola ${escapar(destinatario.nombre || "")}.</p>
  </div>`;
  return { asunto, html };
}
 ...
 * cola tras guardar la llamada. Sólo omite a quien ya recibió el aviso de
 * ESTA MISMA conversación; otra llamada del mismo contacto siempre genera
 * un correo nuevo. Nunca lanza.
 */
export async function notificarLlamada({ clientId, callSid, supabase = getSupabase() }) {
  ...
  try {
    ...
    let lead = null;
    if (llamada.lead_id) {
      lead = (await supabase.from("leads").select("id,nombre").eq("id", llamada.lead_id).eq("client_id", clientId).maybeSingle()).data;
    } else if (llamada.from_number) {
      lead = (await supabase.from("leads").select("id,nombre").eq("client_id", clientId).eq("telefono", llamada.from_number).order("created_at", { ascending: false }).limit(1).maybeSingle()).data;
    }
    /* Se unen todas las entregas parciales de ESTA conversación. La
       clasificación también es una instantánea ligada al call_sid. */
    const [{ data: eventos }, { data: eventosClasificacion }, { data: yaAvisados }] = await Promise.all([
      ...
      supabase.from("lead_events").select("meta,created_at")
        .eq("client_id", clientId).eq("type", "clasificacion_de_llamada")
        .filter("meta->>conversation_id", "eq", callSid).order("created_at", { ascending: false }).limit(1),
      supabase.from("notificaciones_lead").select("email")
        .eq("client_id", clientId).eq("motivo", `llamada:${callSid}`).eq("estado", "enviado"),
    ]);
    const dicho = combinarDatosDeLlamada(eventos || []);
    const metaClasificacion = eventosClasificacion?.[0]?.meta || {};
    const clasificacion = metaClasificacion.departamento || metaClasificacion.motivo
      ? {
          departamento: metaClasificacion.departamento || null,
          nombreDepartamento: metaClasificacion.nombre_departamento || null,
          motivo: metaClasificacion.motivo || "",
        }
      : null;
    ...
    for (const destinatario of destinatarios) {
      ...
      try {
        const { asunto, html } = correoDeLlamada({
          empresa: nombreEmpresa,
          llamada,
          dicho,

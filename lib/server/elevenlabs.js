...
export async function persistElevenLabsCall({
  ...
  const { error: guardarError } = existente
    ? await supabase.from("calls").update(baseCallPayload).eq("id", existente.id)
  ...
  try {
    ...
    console.error("No se pudo anotar el consumo de la llamada:", err?.message || err);
  }
  ...
  if (conversationId) {
    void encolar({
      ...
      unaSolaVez: true,
    }).catch((err) => console.error("No se pudo pedir la copia de la grabación:", err?.message || err));
  }
  ...
  if (conversationId) {
    ...
    }).catch((err) => console.error("No se pudo pedir el aviso de la llamada:", err?.message || err));
  }

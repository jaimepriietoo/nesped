 ...
 * una `<columna>_hash` (HMAC con una clave de búsqueda). Un volcado de la
 * base sin la clave maestra es ilegible; borrar la clave de una empresa es
 * borrar sus datos aunque queden en un backup.
 *
...
/** Una clave por empresa, derivada: borrarla es borrar sus datos. */
...
export function cifrarFila({ tabla, fila, modo, clientId = fila?.client_id, env = process.env }) {
  ...
  for (const columna of columnasPresentes) {
    ...
    if (def.buscables.includes(columna)) salida[`${columna}_hash`] = hashDeBusqueda(columna, valor, env);
...
export function descifrarFila({ tabla, fila, modo, env = process.env, quitar = [] }) {
  ...
  for (const columna of def.cifradas) {
    ...
    if (def.buscables.includes(columna)) delete salida[`${columna}_hash`];
  }
...
export function filtroCifrado(tabla, columna, valor, modo, env = process.env) {
  const def = TABLAS[tabla];
  if (!def || !def.buscables.includes(columna) || (modo !== "cifrado" && modo !== "solo")) return { columna, valor };
  if (Array.isArray(valor)) return { columna: `${columna}_hash`, valor: valor.map((v) => hashDeBusqueda(columna, v, env)) };
  return { columna: `${columna}_hash`, valor: hashDeBusqueda(columna, valor, env) };
}
  ...
  const materializar = () => {
    ...
    for (const [m, args] of llamadas) {
      if (METODOS_FILTRO.has(m) && typeof args[0] === "string") {
        const f = filtroCifrado(tabla, args[0], args[1], modo, env);
...
export function condicionOr(tabla, columnas, valor, modo, env = process.env) {
  return columnas.map((c) => {
    const f = filtroCifrado(tabla, c, valor, modo, env);
    return `${f.columna}.eq.${f.valor}`;
  }).join(",");
}
